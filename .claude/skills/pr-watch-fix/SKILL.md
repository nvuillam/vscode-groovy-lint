...
```
Monitor:
  description: "PR watch: PR #N CI"
  persistent: true
  command: |
    while true; do
      checks="$(gh pr checks N --json name,bucket 2>/dev/null || echo '[]')"
...
If the same check has been pending more than **30 minutes** without a state change, the Monitor must emit a `[stalled]` event and you should **ask the user** whether to keep waiting. (The test matrix is node {22,24} x OS {ubuntu,macos,windows}; each leg downloads a real VS Code via `@vscode/test-electron`, installs subpackage deps with `npm ci`, then runs integration tests under `xvfb-action` - the Windows/macOS legs can take 10-15 min. The VSCodium leg additionally installs VSCodium from a deb repo. Don't panic early.) Do not poll faster than 5 minutes.

### 5. Fix the failures (delegate to `pr-fix`)

Spawn the `pr-fix` agent (Opus) via the Agent tool. Pass it the branch, PR number, current HEAD SHA, and the failure list from `pr-watch` (job names, error types, key log lines). `pr-fix` owns the diagnosis, the fix, local validation (`npm run lint:fix` / `npm run compile` / `npm run dev:pre-commit`; `npm test` only when needed - it downloads VS Code and is heavy), and the commit + push (including the Mega-Linter `--force-with-lease` reconcile and all the git-safety rules).
...
```
Cycle 2: Test (windows-latest, 22) failed (assertion in client/src/test/suite/extension.test.ts:42), pushed e0a44f1. Waiting 5m.
```
...
- Never edit generated files (the `docs/index.md` mirror of `README.md` is regenerated by `npm run dev:pre-commit`; `client/out/` and `server/out/` are gitignored build outputs). Use **npm** not yarn (CI runs `npm ci` and `package-lock.json` is the source of truth). `pr-fix` enforces these; if it reports a violation it could not avoid, stop and ask.