...
import {
  applyQuickFixes,
  applyQuickFixesInFile,
  disableErrorWithComment,
  disableErrorForProject,
} from "./codeActions";
...
export class DocumentsManager {
  ...
  private documentSettings: Map<string, Thenable<VsCodeGroovyLintSettings>> =
    new Map();
  ...
  private docsDiagnostics: Map<string, Diagnostic[]> = new Map<
    string,
    Diagnostic[]
  >();
  ...
  constructor(cnx: any) {
    ...
    if (isTest()) {
      ...
      const config = this.connection.getConfiguration("groovyLint");
      const storedAutoFixTabs = config.get("autoFixTabs");
      if (storedAutoFixTabs === "always") {
        this.autoFixTabs = true;
      } else if (storedAutoFixTabs === "never") {
        this.neverFixTabs = true;
      }
    }
  }
  ...
  async executeCommand(params: any) {
    ...
    if (params.command === COMMAND_LINT.command) {
      const document: TextDocument = this.getDocumentFromUri(
    ...
    // Command: Fix
      ...
      let document: TextDocument = this.getDocumentFromUri(
        this.currentTextDocumentUri
      )!;
  ...
  // Update autoFixTabs in global project configuration
  setAutoFixTabs(autoFixTagFlag: "always" | "never") {
    this.autoFixTabs = autoFixTagFlag === "always";
    this.neverFixTabs = autoFixTagFlag === "never";
    const config = this.connection.getConfiguration("groovyLint");
    config.update("autoFixTabs", autoFixTagFlag);
  }
  ...
  async updateDocumentSettings(
    resource: string,
    settingUpdate: VsCodeGroovyLintSettings
  ) {
  ...
  async validateTextDocument(
    ...
    if (
      opts.displayErrorsEvenIfDocumentClosed !== true &&
      !this.isDocumentOpenInClient(textDocument.uri)
    ) {
      return Promise.resolve([]);
    }
    // Find if document is already being formatted or fixed
    const currentLintsOnDoc = this.currentlyLinted.filter(
      (currLinted) => currLinted.uri === textDocument.uri
    );
    const duplicateLintsOnDoc = currentLintsOnDoc.filter(
      (currLinted) =>
    ...
    if (
      ...
      const justLintedPos = this.currentlyLinted.findIndex(
        (currLinted) =>
          JSON.stringify({
            uri: currLinted.uri,
            options: currLinted.options,
          }) === JSON.stringify({ uri: textDocument.uri, options: opts }) &&
      ...
      const indexNextInQueue = this.queuedLints.findIndex(
        (queuedItem) => queuedItem.uri === textDocument.uri
      );
      ...
      if (indexNextInQueue > -1) {
        ...
        debug(
          `Run queued lint for ${textDocument.uri} (${JSON.stringify(
            lintToProcess.options || "{}"
          )})`
        );
        this.validateTextDocument(textDocument, lintToProcess.options).then(
          async (resVal) => {
            // If format has not been performed by queue request , lint again after it is processed
            if (
              (lintToProcess.options.format === true,
              resVal && resVal.length > 0)
            ) {
    ...
    const currentFormatsOnDoc = currentActionsOnDoc.filter(
      (currLinted) => currLinted.options && currLinted.options.format === true
    );
    const currentFixesOnDoc = currentActionsOnDoc.filter(
      (currLinted) => currLinted.options && currLinted.options.fix === true
    );

    // Format request and no current format or fix: add in queue
    if (
      opts.format === true &&
      currentFormatsOnDoc.length === 0 &&
      currentFixesOnDoc.length === 0
    ) {
    ...
    // Fix request and no current fix: add in queue
      ...
      this.queuedLints.push({ uri: textDocument.uri, options: opts });
      debug(
        `Added in queue: ${textDocument.uri} (${JSON.stringify(opts || "{}")})`
      );
    }
    // All other cases: do not add in queue, else actions would be redundant
    else {
      debug(
        `WE SHOULD NOT BE HERE : ${textDocument.uri} (${JSON.stringify(
          opts || "{}"
        )})`
      );
    }
  ...
  async cancelDocumentValidation(textDocumentUri: string) {
    ...
    this.queuedLints = this.queuedLints.filter(
      (v, i, a) =>
        a.findIndex((t) => JSON.stringify(t) === JSON.stringify(v)) === i
    );
    this.queuedLints = this.queuedLints.filter(
      (queuedLint) => queuedLint.uri !== textDocumentUri
    );
    // Find currently linted document
    this.currentlyLinted = this.currentlyLinted.filter(
      (currLinted) => currLinted.uri !== textDocumentUri
    );
  ...
  async updateDiagnostics(
    ...
    debug(
      `Update diagnostics for ${docUri}: ${diagnostics.length} diagnostics sent`
    );
  ...
  async removeDiagnostics(
    diagnosticsToRemove: Diagnostic[],
    textDocumentUri: string,
    removeAll?: boolean,
    recalculateRangeLinePos?: number
  ): Promise<void> {
    let docDiagnostics: Diagnostic[] =
      this.docsDiagnostics.get(textDocumentUri) || [];
    for (const diagnosticToRemove of diagnosticsToRemove) {
      // Keep only diagnostics not matching diagnosticToRemove ()
      const diagnosticCodeNarcCode = (diagnosticToRemove.code as string).split(
        "-"
      )[0];
